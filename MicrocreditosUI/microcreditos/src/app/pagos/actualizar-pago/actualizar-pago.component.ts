...
export class ActualizarPagoComponent implements OnInit {
  ...
  getPago(): void {
    this.http.get<any>(`http://localhost:5172/api/pago/${this.pagoId}`).subscribe(
      (response) => {
  ...
  onSubmit(): void {
    this.http.put(`http://localhost:5172/api/pago/${this.pagoId}`, this.pagoForm.value).subscribe(
      (response) => {
